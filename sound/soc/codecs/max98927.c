...
#define FADE_MODE_MASK 0x3
#define FADE_IN_TIME_MASK 0xFFC
#define FADE_MUTE_TIME_MASK 0x3FF000
#define FADE_OUT_TIME_MASK 0xFFC00000


#ifdef PA_FADEOUT_PDN
typedef enum {
	DSM_API_MONO_SPKER                  = 0x00000000, /* The mono speaker */
	DSM_API_STEREO_SPKER                = 0x03000000, /* The stereo speakers */

	DSM_API_L_CHAN                      = 0x01000000, /* The left channel speaker */
	DSM_API_R_CHAN                      = 0x02000000, /* The right channel speaker */
...
int reg_common_map[][2] = {
	...
	{MAX98927_Brownout__level_1_current_limit,  0x00},
	{MAX98927_Brownout__level_1_amp_1_control_1,  0x00},
	{MAX98927_Brownout__level_1_amp_1_control_2,  0x00},
	{MAX98927_Brownout__level_1_amp_1_control_3,  0x00},
	{MAX98927_Brownout__level_2_current_limit,  0x00},
	{MAX98927_Brownout__level_2_amp_1_control_1,  0x00},
	{MAX98927_Brownout__level_2_amp_1_control_2,  0x00},
	{MAX98927_Brownout__level_2_amp_1_control_3,  0x00},
	{MAX98927_Brownout__level_3_current_limit,  0x24},
	{MAX98927_Brownout__level_3_amp_1_control_1,  0x00},
	{MAX98927_Brownout__level_3_amp_1_control_2,  0x00},
	{MAX98927_Brownout__level_3_amp_1_control_3,  0x00},
	{MAX98927_Brownout__level_4_current_limit,  0x14},
	{MAX98927_Brownout__level_4_amp_1_control_1,  0x00},
	{MAX98927_Brownout__level_4_amp_1_control_2,  0x00},
	{MAX98927_Brownout__level_4_amp_1_control_3,  0x00},
	{MAX98927_Brownout__level_3_threshold,0x60},
	{MAX98927_Brownout__level_4_threshold,0x58},
...
static bool mutestatus = false;//krayche.yang add for sync mute status 160623
...
static int maxdsm_f0_checking(int* fc_left, int* fc_right)
{
	int ret = 0;
	uint32_t *payload = &gParam[PKG_HEADER];
	int count;
	uint32_t* param;
	int i;
	int rdc_left = 0;
	...
	mutex_lock(&dsm_lock);
	ret = afe_dsm_setget_params((uint8_t* )payload, sizeof(uint32_t)*PAYLOAD_COUNT, 1);
	if (ret != 0) {
		pr_err("%s: afe_dsm_setget_params failed - %d\n", __func__, ret);
	...
	for(i = 0; i < count; i++){
		switch(*param++){
			case ADAPTIVE_FC:						//got fc idx
				*fc_left = *param++;				 //save the fc value of left channel
				*fc_right = *param++;				 //save the fc value of right channel
				break;
			case ADAPTIVE_DC_RES:					//got rdc idx
				rdc_left = *param++;
				rdc_right = *param++;
				break;
			default:
		...
		size_t count, loff_t *ppos)
{
	...
	mutex_lock(&dsm_lock);
	rc = afe_dsm_setget_params(payload, sizeof(uint32_t)*PAYLOAD_COUNT, 1);
	if (rc != 0) {
		pr_err("%s: afe_dsm_setget_params failed - %d\n", __func__, rc);
	}
		...
		size_t count, loff_t *ppos)
{
	...
	afe_dsm_setget_params(payload, count, 0);
exit:
	mutex_unlock(&dsm_lock);
...
#ifdef USE_DSM_MISC_DEV
	maxdsm_f0_checking(&fc_left, &fc_right);
#endif
...
static int fade_in_out_parm_set(const char *val, struct kernel_param *kp)
{
	struct dsm_params* params;
	int ret,mode,fade_in,fade_out,mute;
	param_set_int(val, kp);

	mode = fade_in_out&FADE_MODE_MASK;
	fade_in = (fade_in_out&FADE_IN_TIME_MASK)>>2;
	mute = (fade_in_out&FADE_MUTE_TIME_MASK)>>12;
	fade_out = (fade_in_out&FADE_OUT_TIME_MASK)>>22;
	pr_debug("%s val:%d mode:%d in:%d mute:%d out:%d\n", __func__, fade_in_out, mode, fade_in, mute, fade_out);

	if (1 == mode) { //fade in
		pr_debug("%s, fade_in\n", __func__);
		params = (struct dsm_params *)&gParam[PKG_HEADER];
		ret = 0;
		params->mode = 0;
		params->pcount = 2;
		params->pdata[0] = DSM_SET_STEREO_PARAM(98);
		params->pdata[1] = fade_in;
		params->pdata[2] = DSM_SET_STEREO_PARAM(100);
		params->pdata[3] = mute;
		params->pdata[4] = DSM_SET_STEREO_PARAM(101);
		params->pdata[5] = 1;
		ret = afe_dsm_setget_params((uint8_t*)params, sizeof(dsm_param_t), 0);
		if (ret != 0) {
			pr_err("%s: afe_dsm_setget_params failed - %d\n", __func__, ret);
		}

	} else if (2 == mode) { //fade out -> mute
		pr_debug("%s, fade_out mute\n", __func__);
		params = (struct dsm_params *)&gParam[PKG_HEADER];
		ret = 0;
		params->mode = 0;
		params->pcount = 3;
		params->pdata[0] = DSM_SET_STEREO_PARAM(100);
		params->pdata[1] = mute;
		params->pdata[2] = DSM_SET_STEREO_PARAM(99);
		params->pdata[3] = fade_out;
		params->pdata[4] = DSM_SET_STEREO_PARAM(102);
		params->pdata[5] = 1;
		ret = afe_dsm_setget_params((uint8_t*)params, sizeof(dsm_param_t), 0);
		if (ret != 0) {
			pr_err("%s: afe_dsm_setget_params failed - %d\n", __func__, ret);
		}
	} else if (3 == mode) { //fade out -> mute -> fade in
		pr_debug("%s, fade_out mute fade_in\n", __func__);
		params = (struct dsm_params *)&gParam[PKG_HEADER];
		ret = 0;
		params->mode = 0;
		params->pcount = 5;
		...
		params->pdata[2] = DSM_SET_STEREO_PARAM(99);
		params->pdata[3] = fade_out;
		params->pdata[4] = DSM_SET_STEREO_PARAM(98);
		params->pdata[5] = fade_in;
		params->pdata[6] = DSM_SET_STEREO_PARAM(102);
		params->pdata[7] = 1;
		params->pdata[8] = DSM_SET_STEREO_PARAM(101);
		params->pdata[9] = 1;
		ret = afe_dsm_setget_params((uint8_t*)params, sizeof(dsm_param_t), 0);
		if (ret != 0) {
			pr_err("%s: afe_dsm_setget_params failed - %d\n", __func__, ret);
...
module_param_call(fade_in_out, fade_in_out_parm_set,
		param_get_int, &fade_in_out, 0664);
...
static int max98927_mute(struct snd_soc_dai *dai, int mute)
{
	struct snd_soc_codec *codec = dai->codec;
	struct max98927_priv *max98927 = snd_soc_codec_get_drvdata(codec);
	...
	if(max98927->regmap_l && max98927->left_i2c){
		if(max98927->spk_mode & 0x01){
			mutestatus = 0;//krayche.yang add for sync mute status 160623
			regmap_update_bits(max98927->regmap_l, MAX98927_AMP_enables, 1, 1);
			regmap_update_bits(max98927->regmap_l, MAX98927_Global_Enable, 1, 1);
		}
	}
	if(max98927->regmap_r && max98927->right_i2c){
		if(max98927->spk_mode & 0x02){
			mutestatus = 0;//krayche.yang add for sync mute status 160623
			regmap_update_bits(max98927->regmap_r, MAX98927_AMP_enables, 1, 1);
			regmap_update_bits(max98927->regmap_r, MAX98927_Global_Enable, 1, 1);
		}
	}
	}else{
		mutestatus = 1;//krayche.yang add for sync mute status 160623
		if(max98927->spk_mode != 0){
#ifdef PA_FADEOUT_PDN
...
//krayche.yang add for sync mute status 160623 ++
...
static int max98927_mute_status_set(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct max98927_priv *max98927 = snd_soc_codec_get_drvdata(codec);
...
static const struct snd_kcontrol_new max98927_snd_controls[] = {
	...
	SOC_SINGLE_EXT("BDE AMP EN", MAX98927_Brownout_enables,
	...
	SOC_SINGLE_EXT("DRE EN", MAX98927_DRE_Control,
			MAX98927_DRE_Control_DRE_SHIFT, 1, 0, max98927_reg_get_w, max98927_reg_put_w),
	...
	SOC_SINGLE_EXT("Level3 Threshold", MAX98927_Brownout__level_3_threshold,
			0x60, 255, 0, max98927_reg_get_w, max98927_reg_put_w),
	//0 ~ 255
	SOC_SINGLE_EXT("Level4 Threshold", MAX98927_Brownout__level_4_threshold,
			0x58, 255, 0, max98927_reg_get_w, max98927_reg_put_w),
	//0 ~ 255
	...
	//0:unmute 1:mute krayche.yang add for sync mute status 160623
	SOC_SINGLE_EXT("Mute Status", 0,
			0, 1, 1, max98927_mute_status_get, max98927_mute_status_set),
};

static const struct snd_soc_dapm_route max98927_audio_map[] = {
	{"BE_OUT", NULL, "DAI_OUT"},
	{"iv_feedback", NULL, "BE_IN"},
};
...
int probe_common(struct i2c_client *i2c, struct max98927_priv *max98927) {
	...
	if (ret != 0)
		pr_err("max98927 misc_register dsm error:%d\n", ret);
#endif
		...
		const struct i2c_device_id *id)
{
	...
	if (!max98927) {
		...
		max98927->reset_gpio = of_get_named_gpio_flags(i2c->dev.of_node, "maxim,98927-reset-gpio", 0, NULL);
		ret = gpio_request(max98927->reset_gpio, "max_98927_reset");
		if (ret) {
			pr_err("max98927_i2c_probe : failed to request rest gpio %d error:%d\n",
					max98927->reset_gpio, ret);
